...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const { tasks, events, clients, transactions, documents, metrics, loadedFiles, suggestedFiles } = data;
  ...
  const loadFile = useCallback((path: string, content: string) => {
    setData((prev: DataState) => {
      const existing = prev.loadedFiles.find((f) => f.path === path);
      const now = new Date().toISOString();
      if (existing) {
        return {
          ...prev,
          loadedFiles: prev.loadedFiles.map((f) =>
            f.path === path ? { ...f, content, loadedAt: now } : f
          ),
        };
      }
      return {
  ...
  const unloadFile = useCallback((path: string) => {
    setData((prev: DataState) => ({
      ...prev,
      loadedFiles: prev.loadedFiles.filter((f) => f.path !== path),
    }));
  }, []);
  ...
  const markFileAsRead = useCallback((path: string) => {
    setData((prev: DataState) => ({
      ...
      loadedFiles: prev.loadedFiles.map((f) =>
        f.path === path
          ? { ...f, lastReadAt: new Date().toISOString(), readCount: f.readCount + 1 }
          : f
      ),
    }));
  }, []);
  ...
  const clearLoadedFiles = useCallback(() => {
    setData((prev: DataState) => ({ ...prev, loadedFiles: [] }));
  }, []);
  ...
  const updateSuggestions = useCallback((taskTitle: string, taskDescription?: string) => {
    ...
    setData((prev: DataState) => ({
      ...prev,
      suggestedFiles: suggestions.slice(0, 5),
    }));
  }, [loadedFiles]);

  const clearSuggestions = useCallback(() => {
    setData((prev: DataState) => ({ ...prev, suggestedFiles: [] }));
  }, []);
  ...
  const value = useMemo(() => ({
    ...
    // Vault
    loadedFiles, suggestedFiles,
    loadFile, unloadFile, markFileAsRead, clearLoadedFiles,
    updateSuggestions, clearSuggestions,
    isFileLoaded, getFileContent,
  }), [
    tasks, addTask, updateTask, deleteTask, moveTaskToStatus,
    getTasksByStatus, getTasksByBusiness, getTasksByClient, getFilteredTasks,
    getTodayTasks, getOverdueTasks, getUpcomingTasks, getInboxTasks, getWaitingTasks,
